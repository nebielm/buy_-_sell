...
def create_user(image: Annotated[UploadFile, File()] = None, db: Session = Depends(get_db),
    ...
    if image:
        ...
        default_download_link = ("https://buysellusers.s3.eu-north-1.amazonaws.com/019199fa-8037-"
                                 "7d70-889d-e5738feb4bd7_28_08_2024_19_13_36_"
...
def update_user(user_id: int, image: Annotated[UploadFile, File()] = None,
    ...
    if (user.email and c_user.get_user_by_email(db=db, user_email=user.email)
            and user.email != db_user.email):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Email already registered"
        )
    if (user.username and c_user.get_user_by_username(db=db, username=user.username)
            ...
            if post_pictures:
                for picture in post_pictures:
                    ...
                    if "default_post_pic.jpg" not in image_name:
                        utils.delete_image_from_s3(object_name=image_name,
                                                   bucket_name=BUCKET_NAME_POST_PIC)
