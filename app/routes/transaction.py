...
@router.post("/users/{user_id}/post/{post_id}/transaction/",
             response_model=s_transaction.Transaction)
def create_transaction(user_id: int, post_id: int,
...
@router.get("/users/{user_id}/post/{post_id}/transaction/",
...
def get_transaction_by_post_id(user_id: int, post_id: int, db: Session = Depends(get_db),
    ...
    for transaction in db_transactions:
        if user_id in (transaction.buyer_id, transaction.seller_id):
...
@router.get("/users/{user_id}/transaction/{transaction_id}/",
            response_model=s_transaction.Transaction)
def get_transaction_by_transaction_id(user_id: int, transaction_id: int,
    ...
    if user_id not in (db_transaction.buyer_id, db_transaction.seller_id):
        raise HTTPException(
...
@router.get("/users/{user_id}/received_transaction/",
            response_model=list[s_transaction.Transaction])
...
@router.put("/users/{user_id}/transaction/{transaction_id}/",
            response_model=s_transaction.Transaction)
def update_transaction(user_id: int, transaction_id: int,
    ...
    return c_transaction.update_transaction(db=db, transaction_id=transaction_id,
