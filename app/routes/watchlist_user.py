...
@router.get("/following_user/{following_user_id}/watchlist/{watch_user_id}/", response_model=s_watch_user.WatchUser)
def get_watch_user_record_by_watch_user_id(following_user_id: int, watch_user_id: int, db: Session = Depends(get_db),
    ...
    if not db_watch_user or db_watch_user.following_user_id != following_user_id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail="Authentication failed or "
                                   "Watch user record does not exist")
    return db_watch_user


@router.get("/followed_user/{followed_user_id}/watchlist/", response_model=list[s_watch_user.WatchUser])
def get_watchlist_user_by_followed_user(followed_user_id: int, db: Session = Depends(get_db),
    ...
    if followed_user_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,
                            detail="Authentication failed")
    db_watch_user = c_watch_user.get_watchlist_user_by_followed_user_id(db=db, followed_user_id=followed_user_id)
    if not db_watch_user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
...
@router.get("/following_user/{following_user_id}/watchlist/", response_model=list[s_watch_user.WatchUser])
def get_watchlist_user_by_following_user(following_user_id: int, db: Session = Depends(get_db),
                                         current_user: m_user.User = Depends(get_current_user)):
    if following_user_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,
                            detail="Authentication failed")
    db_watch_user = c_watch_user.get_watchlist_user_by_following_user_id(db=db, following_user_id=following_user_id)
    if not db_watch_user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
...
def delete_watch_user_record(following_user_id: int, watch_user_id: int, db: Session = Depends(get_db),
                             current_user: m_user.User = Depends(get_current_user)):
    if following_user_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,
                            detail="Authentication failed")
